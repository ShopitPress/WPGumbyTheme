<?php
	...
	if (!empty( $previous_post_url ) || !empty( $next_post_url )){
		...
		if (!empty( $next_post_url )):
			...
			echo '<a href="' . $next_post_url . '"><i class="icon-left-open"></i> ' . __( 'Newer post', 'wpgumby' ) . '</a>';
			echo '</div> ';
		endif;
		if (!empty( $previous_post_url )):
			echo ' <div class="medium btn pill-right secondary">';
			echo '<a href="' . $previous_post_url . '">' . __( 'Older post', 'wpgumby' ) . ' <i class="icon-right-open"></i></a>';
			echo '</div>';
		endif;
		echo "</div>";
	}
