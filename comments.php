...
<div id="respond">
	...
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
		<?php if (is_user_logged_in()) : ?>
			...
			&nbsp; <span class="danger badge"><a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php __('Log out of this account', 'wpgumby'); ?>" ><?php _e('Log out &raquo;', 'wpgumby'); ?></a></span>
			</p>
            <div class="row">
		<?php else : ?>
        ...
        <?php endif; //is_user_logged_in() ?>
        	<ul class="<?php if (is_user_logged_in()) { echo "twelve"; } else { echo "six"; }  ?> columns">
				<li class="field">
                <textarea name="comment" id="comment" class="input textarea" rows="5" aria-required="true" placeholder="<?php _e('Comment', 'roots'); ?>"></textarea>
                </li>
                <li>
                	<div class="medium default btn"><input name="submit" type="submit" id="submit" value="<?php _e('Submit Comment', 'roots'); ?>"></div>
