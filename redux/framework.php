 ...
 *
 * @package     Redux_Framework (customized for wpGumby)
 * @subpackage  Core
 * @author      Redux Framework Team
 * @version     3.1.5 (2014.02.21)
 */
...
/**
        Still need to port these.
            ...
            $defaults['footer_credit']      = '<span id="footer-thankyou">' . __( 'Options panel created using', 'wpgumby') . ' <a href="' . $this->framework_url . '" target="_blank">' . __('Redux Framework', 'wpgumby') . '</a> v' . self::$_version . '</span>';
            $defaults['help_tabs']          = array();
            $defaults['help_sidebar']       = ''; // __( '', 'wpgumby' );
...
**/
        ...
        public $args                = array(
            ...
            'domain'             => 'wpgumby', // Translation domain key
        ...
        public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
            ...
            if ( empty( $this->args['footer_credit'] ) ) {
                $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'wpgumby' ), '<a href="'.esc_url( $this->framework_url ).'" target="_blank">'.__( 'Redux Framework', 'wpgumby' ).'</a> v'.self::$_version ) . '</span>';
            }
            if ( empty( $this->args['menu_title'] ) ) {
                $this->args['menu_title'] = __( 'Options', 'wpgumby' );
            }
            if ( empty( $this->args['page_title'] ) ) {
                $this->args['page_title'] = __( 'Options', 'wpgumby' );
            }
        ...
        public function _internationalization() {
             ...
             * @param string     'wpgumby'  text domain
             */
            $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'wpgumby' );
            ...
            load_textdomain( 'wpgumby', dirname( __FILE__ ) . '/languages/' . $locale . '.mo' );
...
//                $comparison = $field['required'][1];
        ...
        function _admin_bar_menu()
        {
            ...
            if($menu){
                ...
                $nodeargs = array(
                    ...
                    'title' => "<span class='ab-icon dashicons-admin-generic'></span>" . $theme_data->get('Name') . " " . __('Options', 'wpgumby'),
        ...
        public function _enqueue() {
            ...
            $this->localize_data['args'] = array(
                'save_pending'          => __( 'You have changes that are not saved. Would you like to save them now?', 'wpgumby' ), 
                'reset_confirm'         => __( 'Are you sure? Resetting will lose all custom values.', 'wpgumby' ), 
                'reset_section_confirm' => __( 'Are you sure? Resetting will lose all custom values in this section.', 'wpgumby' ), 
                'preset_confirm'        => __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'wpgumby' ), 
        ...
        public function get_default_output_string($field) {
          ...
          if ( !empty( $default_output ) ) {
              $default_output = __( 'Default', 'wpgumby' ) . ": " . substr($default_output, 0, -2);
          }           
          if (!empty($default_output)) {
        ...
        public function _options_page_html() {
            ...
            // Do we support JS?
            echo '<noscript><div class="no-js">' . __( 'Warning- This options panel will not work properly without javascript!', 'wpgumby' ) . '</div></noscript>';
            ...
            echo '<a href="javascript:void(0);" class="expand_options">' . __( 'Expand', 'wpgumby' ) . '</a>';
            echo '<div class="redux-action_bar">';
            submit_button( __( 'Save Changes', 'wpgumby'), 'primary', 'redux_save', false );
            echo '&nbsp;';
            submit_button( __( 'Reset Section', 'wpgumby'), 'secondary', $this->args['opt_name'] . '[defaults-section]', false );
            echo '&nbsp;';            
            submit_button( __( 'Reset All', 'wpgumby' ), 'secondary', $this->args['opt_name'] . '[defaults]', false );
            echo '</div>';

            echo '<div class="redux-ajax-loading" alt="' . __( 'Working...', 'wpgumby' ) . '">&nbsp;</div>';
            echo '<div class="clear"></div>';
            echo '</div>';
            ...
            if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && $saved == '1' ) {
                if( isset( $this->options['REDUX_imported'] ) && $this->options['REDUX_imported'] === 1 ) {
                     ...
                     */
                    echo '<div id="redux-imported"><strong>' . apply_filters( "redux-imported-text-{$this->args['opt_name']}", __( 'Settings Imported!', 'wpgumby' ) ) . '</strong></div>';
                } else {
                    /**
                     * filter 'redux-saved-text-{opt_name}'
                     * @param string translated "settings saved" text
                     */
                    echo '<div id="redux-save"><strong>' . apply_filters( "redux-saved-text-{$this->args['opt_name']}", __( 'Settings Saved!', 'wpgumby' ) ) . '</strong></div>';
                }
            }
            /**
             ...
             */
            echo '<div id="redux-save-warn"><strong>' . apply_filters( "redux-changed-text-{$this->args['opt_name']}", __( 'Settings have changed, you should save them!', 'wpgumby' ) ) . '</strong></div>';
            echo '<div id="redux-field-errors"><strong><span></span> ' . __( 'error(s) were found!', 'wpgumby' ) . '</strong></div>';
            echo '<div id="redux-field-warnings"><strong><span></span> ' . __( 'warning(s) were found!', 'wpgumby' ) . '</strong></div>';
            ...
            if( $this->args['show_import_export'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span class="group_title">' . __( 'Import / Export', 'wpgumby' ) . '</span></a>';
                echo '</li>';
            ...
            if( $this->args['dev_mode'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span class="group_title">' . __( 'Options Object', 'wpgumby' ) . '</span></a>';
                echo '</li>';
            }
            ...
            if( $this->args['system_info'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="system_info_default_section_group_li_a" class="redux-group-tab-link-a custom-tab" data-rel="system_info_default">' . $icon . ' <span class="group_title">' . __( 'System Info', 'wpgumby' ) . '</span></a>';
                echo '</li>';
            }
            ...
            if( $this->args['show_import_export'] === true ) {
                ...
                echo '<h3>' . __( 'Import / Export Options', 'wpgumby' ) . '</h3>';
                echo '<h4>' . __( 'Import Options', 'wpgumby' ) . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-import-code-button" class="button-secondary">' . __( 'Import from file', 'wpgumby' ) . '</a> <a href="javascript:void(0);" id="redux-import-link-button" class="button-secondary">' . __( 'Import from URL', 'wpgumby' ) . '</a></p>';
                 ...
                 */
                echo '<p class="description" id="import-code-description">' . apply_filters( 'redux-import-file-description', __( 'Input your backup file below and hit Import to restore your sites options from a backup.', 'wpgumby' ) ) . '</p>';
                echo '</div>';
                 ...
                 * @param string translated settings import instructions text
                 */
                echo '<p class="description" id="import-link-description">' . apply_filters( 'redux-import-link-description', __( 'Input the URL to another sites options set and hit Import to load the options from that site.', 'wpgumby' ) ) . '</p>';
                echo '</div>';
                 ...
                 */
                echo '<p id="redux-import-action"><input type="submit" id="redux-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __( 'Import', 'wpgumby' ) . '">&nbsp;&nbsp;<span>' . apply_filters( 'redux-import-warning', __( 'WARNING! This will overwrite all existing option values, please proceed with caution!', 'wpgumby' ) ) . '</span></p>';
                echo '<div class="hr"/><div class="inner"><span>&nbsp;</span></div></div>';

                echo '<h4>' . __( 'Export Options', 'wpgumby' ) . '</h4>';
                echo '<div class="redux-section-desc">';
                /**
                 * filter 'redux-backup-description'
                 * @param string translated settings backup information
                 */
                echo '<p class="description">' . apply_filters( 'redux-backup-description', __( 'Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'wpgumby' ) ) . '</p>';
                echo '</div>';


                /** @noinspection PhpUndefinedConstantInspection */
                echo '<p><a href="javascript:void(0);" id="redux-export-code-copy" class="button-secondary">' . __( 'Copy', 'wpgumby' ) . '</a> <a href="' . add_query_arg( array( 'feed' => 'redux_options_' . $this->args['opt_name'], 'action' => 'download_options', 'secret' => md5( AUTH_KEY . SECURE_AUTH_KEY ) ), site_url() ) . '" id="redux-export-code-dl" class="button-primary">' . __( 'Download', 'wpgumby' ) . '</a> <a href="javascript:void(0);" id="redux-export-link" class="button-secondary">' . __( 'Copy Link', 'wpgumby' ) . '</a></p>';
                $backup_options = $this->options;
                $backup_options['redux-backup'] = '1';
            ...
            if( $this->args['dev_mode'] === true ) {
                ...
                echo '<h3>' . __( 'Options Object', 'wpgumby' ) . '</h3>';
                echo '<div class="redux-section-desc">';
                ...
                echo '<a href="#" id="consolePrintObject" class="button">' . __( 'Show Object in Javascript Console Object', 'wpgumby' ) . '</a>';
            ...
            if( $this->args['system_info'] === true ) {
                ...
                echo '<h3>' . __( 'System Info', 'wpgumby' ) . '</h3>';
