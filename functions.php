...
function wpgumby_setup() {
	...
	function wp_nav_menu_args( $args = '' ) { $args['container'] = false; return $args; }
	...
	if ( function_exists( 'add_image_size' ) ) { 
		...
		add_image_size( 'portfolio-thumb', 460, 460, true );
	}
}
...
function wpgumby_scripts_styles() {
	...
	wp_enqueue_script( 'wpgumby-camera-fix', get_template_directory_uri() . '/js/camera-fix.js', array(), '2.8.3', false );
	wp_enqueue_script( 'jquery' ); //Enqueue from wordpress
	wp_enqueue_script( 'wpgumby-gumby', get_template_directory_uri() . '/js/gumby-2.6.0.js', array(), '2.6.0', true );
	wp_enqueue_script( 'wpgumby-gumbyui', get_template_directory_uri() . '/js/gumby.ui.plugins-2.6.0.js', array(), '2.6.0', true );
	wp_enqueue_script( 'wpgumby-plugins', get_template_directory_uri() . '/js/plugins.js', array(), '1.0.0', true );
	wp_enqueue_script( 'jquery-easing', get_template_directory_uri() . '/js/jquery.easing.1.3.js', array(), '1.3.0', true );
	wp_enqueue_script( 'wpgumby-camera', get_template_directory_uri() . '/js/camera-1.3.4.js', array(), '1.3.4', false );
	wp_enqueue_script( 'wpgumby-main', get_template_directory_uri() . '/js/main.js', array(), '0.0.1', true );
	...
	wp_enqueue_style( 'wpgumby-wpgumbycss', get_template_directory_uri() . '/css/wpgumby-2.6.0.css' );
	if($wpgumby_data['width_radio'] == 'full_width'){
...
function wpgumby_widgets_init() {
	register_sidebar( array(
		...
		'id' => 'sidebar-1',
		'description' => __( 'Some text.', 'wpgumby' ),
...
function wpgumby_options_box_content( $post ) {
	...
	($slider) ? $status = 'checked="checked"' : $status = '';
	echo '  <input name="slidersh" type="checkbox" value="show" '.$status.'>';
	$sidebars = array('no_sidebar' => __('No Sidebar', 'wpgumby'),
					  '2c-l' => __('Left', 'wpgumby'),
					  '2c-r' => __('Right', 'wpgumby')
					  );
	$sidebar = get_post_meta( $post->ID, 'p_sidebar', true);
	echo '<p><label for="model data">'.__('Sidebar options', 'wpgumby').':</label>';
	$soptions = '<option value="">'.__('Default', 'wpgumby').'</option>';
	foreach ($sidebars as $key => $value){
		($key == $sidebar) ? $status = 'selected="selected"' : $status = '';
		$soptions .= '<option value="'.$key.'" '.$status.'>'.$value.'</option>';
	}
	echo '<select name="p_sidebar">'.$soptions.'</select>';
}
...
function wpgumby_options_box_save( $post_id ) {
	...
	if (isset($_POST['p_sidebar'])) { $sidebar_status = $_POST['p_sidebar']; } else { $sidebar_status = ""; }
	update_post_meta( $post_id, 'slider', $slider_status );
	update_post_meta( $post_id, 'p_sidebar', $sidebar_status );
}
...
function wpgumby_comment($comment, $args, $depth) {
	...
	<div class="comment-author-comment nine columns">
		...
		<?php echo get_comment_author_link(); ?> on <a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>"><?php printf( __('%1$s at %2$s', 'wpgumby'), get_comment_date(),  get_comment_time()); ?></a> &nbsp; <?php edit_comment_link(__('<span class="default badge">Edit</span>', 'wpgumby'),'  ','' );?> &nbsp; 
		<?php
...
function wpgumby_pings($comment, $args, $depth) {
	...
	<div class="comment-author-comment twelve columns">
		...
		<?php echo get_comment_author_link(); ?> on <a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>"><?php printf( __('%1$s at %2$s', 'wpgumby'), get_comment_date(),  get_comment_time()); ?></a> &nbsp; <?php edit_comment_link(__('<span class="light badge">Edit</span>', 'wpgumby'),'  ','' );?> &nbsp; 
		</div>
        <div class="ping-text">
		<?php comment_text() ?>
        </div>
...
function wpgumby_get_category_tags($args) {
	...
	foreach ($tags as $tag) {
		...
		$terms = get_term($tag->tag_id, 'post_tag');
		$tags[$count]->tag_slug = $terms->slug;
		$count++;
	}
	return $tags;
}
...
function wpgumby_breadcrumbs() {
	...
	if (is_home() || is_front_page()) {
		if ($show_on_home == 1) echo '<div class="breadcrumbs row"><a href="' . $home_link . '">' . $text['home'] . '</a></div>';
	} else {
...
//wp_title() Filter
if (!function_exists( 'wpgumby_wp_title' )) :
	function wpgumby_wp_title( $title, $sep ) {
		global $page, $paged;
		if ( is_feed() ) { return $title; }
		$title .= get_bloginfo( 'name' );
		$site_description = get_bloginfo( 'description', 'display' );
		if ( $site_description && ( is_home() || is_front_page() ) ) { $title .= " $sep $site_description"; }
		if ( $paged >= 2 || $page >= 2 ) { $title .= " $sep " . sprintf( __( 'Page %s', 'wpgumby' ), max( $paged, $page ) ); }
		return $title;
	}
	add_filter( 'wp_title', 'wpgumby_wp_title', 10, 2 );
endif;
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	//Custom Gravatar
	function wpgumby_wc_gravatar() {
		add_filter('woocommerce_review_gravatar_size', 'custom_woocommerce_gravatar_src');
		function custom_woocommerce_gravatar_src( $size ) { $size = 140; return $size; }
	}
	add_action( 'init', 'wpgumby_wc_gravatar' );
